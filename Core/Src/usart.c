...
uint8_t USART2_Received_Length=9;
/* USER CODE END 0 */
...
void HAL_UART_MspDeInit(UART_HandleTypeDef* uartHandle)
{
  ...
  /* USER CODE END USART2_MspDeInit 0 */
    ...
    PA3     ------> USART2_RX
    */
    HAL_GPIO_DeInit(GPIOA, miniboard1_Pin|miniboard2_Pin);
...
void Usart1_Display_Data_Value(uint16_t Data)
{
	char buffer[7];
	uint32_t temp=Data;
	buffer[0]=temp/10000+'0';
	buffer[1]=(temp/1000)%10+'0';
	buffer[2]=(temp/100)%10+'0';
	buffer[3]=(temp/10)%10+'0';
	buffer[4]=temp%10+'0';
	buffer[5]='\r';
	buffer[6]='\t';
...
void Usart1_Display_String(char* String)
{
...
/**
  ...
  * @retval None
  */
void My_Usart3_Config(uint32_t RS232_BaudRate)
{
...
#ifdef CRC16_CHECK
		uint16_t CrcValue=((USART1_Receive_Data [USART1_Received_Length-2])|((uint16_t)(USART1_Receive_Data[USART1_Received_Length -1])<<8));
		//接收端用CRC16算法对接到的数据进行校验，计算出的结果和传过来的CRC字节比较是否相等
		if(CrcValue==CRC16(USART1_Receive_Data,USART1_Received_Length-2 ))
		{
				//通过CRC16校验，判断具体的指令执行具体的动
				switch(USART1_Receive_Data [1])
				{
					case 0:function00();break;//test
